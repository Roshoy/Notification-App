...
public class AddUserController {
    ...
    private void handleAddAction(ActionEvent actionEvent) {
        ...
        if (loginTextField.getText().isEmpty() || firstNameTextField.getText().isEmpty() ||
                lastNameTextField.getText().isEmpty() || passwordTextField.getText().isEmpty() ||
                (!standardUserRadioButton.isSelected() && !coordinatorRadioButton.isSelected())) {
        ...
        if (standardUserRadioButton.isSelected()) {
            Optional<User> newUser = User.createUserAccount(loginTextField.getText(), firstNameTextField.getText(),
            ...
            Optional<Coordinator> newCoordinator = Coordinator.createCoordinatorAccount(loginTextField.getText(),
                    firstNameTextField.getText(), lastNameTextField.getText(), passwordTextField.getText(),
                    Department.findIdByName(departmentField.getSelectionModel().getSelectedItem()));
            if (newCoordinator.equals(Optional.empty())) {
                new Alert(Alert.AlertType.ERROR, "Coordinator with that login already exists!").showAndWait();
                loginTextField.setText("");
                return;
            }
